...
class _StatisticsPageState extends State<StatisticsPage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => HomeStatsBloc(HomeStatsRepo())..add(const FetchHomeStats()),
      child: Container(
                        ...
                        return RefreshIndicator(
                          onRefresh: () async {
                            try {
                              ...
                              bloc.add(const RefreshHomeStats());
                            } catch (e) {
                              //
                            }
                          },
                          child: BlocBuilder<HomeStatsBloc, HomeStatsState>(
                            builder: (context, homeStatsState) {
                              if (homeStatsState.status == ApiStatus.loading) {
                                return const Center(child: LoadingWidget());
                              } else if (homeStatsState.status == ApiStatus.failed) {
                                return EmptyStateWidget.noData(
                                  onRetry: () {
                                    context.read<HomeStatsBloc>().add(const FetchHomeStats());
                                  },
                                );
                              } else if (homeStatsState.status == ApiStatus.success &&
                                  homeStatsState.response != null) {
                                return _buildHomeStatsContent(homeStatsState.response!).fadeAndSlideAnimation();
                              } else {
                                return Center(
                                  child: CustomText(
  ...
  Widget _buildProfileCard(HomeStatsResponse response) {
    final profileData = _findDataByKey(response.data, 'profile', (data) => data.profileData);

    return CustomCard(
      onTap: () {
        context.push(AppRoutes.profile);
      },
      padding: EdgeInsets.all((isTablet() ? 15 : 24).w),
        ...
        children: [
            ...
            child: Stack(
              children: [
                BlocBuilder<ProfileBloc, ProfileState>(
                  builder: (context, state) {
                    String profileImageUrl = state.profile?.user?.profileImage ?? "";

                    return CustomAvatar(
                      radius: isTablet() ? 90 : 45,
                ...
                Positioned(
                  bottom: (isTablet() ? 5 : 12).h,
                  right: 5.w,
                  child: Container(
                    width: (isTablet() ? 15 : 10).w,
                    height: (isTablet() ? 15 : 10).h,
                    decoration: BoxDecoration(
                      color:
                          profileData?.deliveryBoy.status == 'active' ? AppColors.successColor : AppColors.errorColor,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                CustomText(
                  text: profileData?.deliveryBoy.fullName ?? AppLocalizations.of(context)!.deliveryPartner,
                  fontSize: sz(20, seprateTabletSize: 14),
                  fontWeight: FontWeight.bold,
                ),
                SizedBox(height: 4.h),
                CustomText(
                  text: AppLocalizations.of(context)!.deliveryPartner,
                  fontSize: sz(14, seprateTabletSize: 9),
                  color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.6),
                ),
                SizedBox(height: 8.h),
                Row(
                  children: [
                    Icon(Icons.star, color: AppColors.accentYellow, size: (isTablet() ? 10 : 16).sp),
                    const SizedBox(width: 4),
                    CustomText(text: '${profileData?.deliveryBoy.rating ?? 0.0}'),
                    const SizedBox(width: 16),
                    Icon(Icons.local_shipping, color: AppColors.accentBlue, size: (isTablet() ? 10 : 16).sp),
                    const SizedBox(width: 4),
                    CustomText(
                      text:
  ...
  Widget _buildSummaryCards(HomeStatsResponse response) {
    final summaryData = _findDataByKey(response.data, 'summary', (data) => data.summaryData);
          ...
          children: [
            Expanded(
              child: _buildStatCard(
                ...
                CurrencyFormatter.formatAmount(context, (summaryData?.today.earnings ?? 0.0).toString()),
                Icons.today,
            ...
            Expanded(
              child: _buildStatCard(
                ...
                CurrencyFormatter.formatAmount(context, (summaryData?.thisWeek.earnings ?? 0.0).toString()),
                Icons.weekend,
          ...
          children: [
            Expanded(
              child: _buildStatCard(
                ...
                CurrencyFormatter.formatAmount(context, (summaryData?.thisMonth.earnings ?? 0.0).toString()),
            ...
            Expanded(
              child: _buildStatCard(
                ...
                CurrencyFormatter.formatAmount(context, (summaryData?.total.earnings ?? 0.0).toString()),
  ...
  Widget _buildStatCard(String title, String amount, IconData icon, Color color) {
    return CustomCard(
      ...
      child: Column(
        children: [
          Icon(icon, color: color, size: (isTablet() ? 22 : 20).sp),

          // Icon(icon, color: color, size: 24),
          SizedBox(height: 8.h),
          CustomText(text: title, fontWeight: FontWeight.w500),
          SizedBox(height: 4.h),
          CustomText(text: amount, fontWeight: FontWeight.bold),
        ],
      ),
    );
  }
  ...
  Widget _buildPerformanceMetrics(HomeStatsResponse response) {
    final performanceData = _findDataByKey(response.data, 'performanceMetrics', (data) => data.performanceMetricsData);
              ...
              children: [
                ...
                Expanded(
                  child: _buildMetricItem(
                    ...
                    () => context.push('/ratings'), // Clickable - navigates to ratings
                  ),
                ),
              ],
            ),
  ...
  Widget _buildMetricItem(String title, String value, IconData icon, Color color, VoidCallback? onTap) {
  ...
  Widget _buildTodayProgress(HomeStatsResponse response) {
    final todayData = _findDataByKey(response.data, 'todayProgress', (data) => data.todayProgressData);
            ...
            children: [
              Expanded(
                child: _buildProgressItem(
                  ...
                  CurrencyFormatter.formatAmount(context, (todayData?.earnings ?? 0.0).toString()),
                  Icons.money,
                  Colors.green,
                ),
              ),
  ...
  Widget _buildEarningsAnalytics(HomeStatsResponse response) {
    final analyticsData = _findDataByKey(response.data, 'earningsAnalytics', (data) => data.value);
  ...
  Widget _buildEarningsChart(dynamic charts) {
    ...
    try {
      ...
      return SizedBox(
        height: 250.h,
        child: Center(child: CustomText(text: AppLocalizations.of(context)!.errorLoadingChartData, color: Colors.grey)),
      );
    }
    ...
    if (flSpotData.isEmpty) {
      return SizedBox(
        ...
        child: Center(
          child: CustomText(
            text: AppLocalizations.of(context)!.noDataAvailableForPeriod(_selectedChartPeriod),
            color: Colors.grey,
          ),
        ),
      );
    ...
    double maxY = flSpotData.isEmpty ? 5000 : flSpotData.map((e) => e.y).reduce((a, b) => a > b ? a : b);
        ...
        LineChartData(
                ...
                getTitlesWidget: (double value, TitleMeta meta) {
                  ...
                  if (index >= 0 && index < labels.length) {
                    return Padding(
                      ...
                      child: CustomText(
                        text: labels[index],
                        fontSize: sz(12, seprateTabletSize: 8),
              ...
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (double value, TitleMeta meta) {
                  ...
                  return CustomText(
                    ...
                    fontSize: sz(10, seprateTabletSize: 7),
                    color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.7),
                    fontWeight: FontWeight.w500,
                  );
                },
                reservedSize: isTablet() ? 70 : 35,
              ),
            ),
          ),
          ...
          lineBarsData: [
            LineChartBarData(
              spots: flSpotData,
              isCurved: true,
              gradient: LinearGradient(colors: [AppColors.primaryColor, AppColors.primaryColor.withValues(alpha: 0.8)]),
              barWidth: 3,
      ...
      children: [
        Expanded(
          child: Container(
            ...
            child: Row(
              children: [
                // Icon(icon, color: color, size: (isTablet() ? 22 : 20).sp),
                Icon(TablerIcons.moneybag, color: AppColors.primaryColor, size: (isTablet() ? 22 : 16).sp),
                ...
                Column(
                  ...
                  children: [
                    ...
                    CustomText(
                      text: CurrencyFormatter.formatAmount(context, totalEarnings.toString()),
                      fontSize: sz(18, seprateTabletSize: 14),
                      fontWeight: FontWeight.bold,
                      color: Theme.of(context).colorScheme.onSurface,
                    ),
        ...
        Expanded(
          child: Container(
            ...
            child: Row(
              children: [
                Icon(TablerIcons.trending_up, color: AppColors.primaryColor, size: (isTablet() ? 22 : 16).sp),

                // const Icon(Icons.trending_up, color: AppColors.primaryColor, size: 16),
                SizedBox(width: 8.w),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    SizedBox(height: 4.h),
                    CustomText(
                      text: CurrencyFormatter.formatAmount(context, averageEarnings.toString()),
                      fontSize: sz(18, seprateTabletSize: 14),
                      fontWeight: FontWeight.bold,
                      color: Theme.of(context).colorScheme.onSurface,
                    ),
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Container(
              padding: EdgeInsets.all((isTablet() ? 7 : 16).w),
              decoration: BoxDecoration(
              ...
              child: Row(
                children: [
                  Container(
                    padding: EdgeInsets.all((isTablet() ? 5 : 12).w),
                    decoration: BoxDecoration(
                      color: AppColors.accentBlue.withValues(alpha: 0.2),
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Icon(Icons.refresh, color: AppColors.accentBlue, size: (isTablet() ? 22 : 16).sp),
                  ),
                  SizedBox(width: 16.w),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        CustomText(text: AppLocalizations.of(context)!.viewHistory, fontWeight: FontWeight.w600),
                        SizedBox(height: 4.h),
                        CustomText(
                          text: AppLocalizations.of(context)!.checkPastDeliveries,
                          fontSize: sz(12, seprateTabletSize: 8),
                          color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.6),
                        ),
                      ],
  ...
  T? _findDataByKey<T>(List<HomeStatsData> data, String key, T Function(HomeStatsData) extractor) {
    try {
