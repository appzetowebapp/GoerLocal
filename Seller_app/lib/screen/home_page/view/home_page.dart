...
class _HomePageState extends State<HomePage> {
          ...
          body: MultiBlocListener(
            listeners: [
              BlocListener<StoreSwitcherCubit, StoreSwitcherState>(
                listenWhen: (previous, current) => previous.selectedStore?.id != current.selectedStore?.id,
                listener: (context, state) {
                  if (state.selectedStore != null) {
                    context.read<HomePageBloc>().add(FetchHomePageData(storeId: state.selectedStore!.id));
                    context.read<OrdersBloc>().add(LoadOrdersInitial(storeId: state.selectedStore!.id));
                    if (HiveStorage.isSubscriptionAvailable) {
                      context.read<CurrentSubscriptionBloc>().add(FetchCurrentSubscription());
                    }
                  }
  ...
  Widget _buildHeader(BuildContext context, ScreenType screenType, AppLocalizations? l10n) {
              ...
              children: [
                state.isLoading
                    ? CustomShimmer(
                        ...
                        height: UIUtils.profileAvatarRadius(screenType) * 2 - 20,
                        borderRadius: BorderRadius.circular(100),
                      )
                    : ClipOval(
                        child: SizedBox(
                          width: (UIUtils.profileAvatarRadius(screenType) - 10) * 2,
                          height: (UIUtils.profileAvatarRadius(screenType) - 10) * 2,
                          child: store != null && (store.logo?.isNotEmpty ?? false)
                              ? CustomNetworkImage(
                                  imageUrl: store.logo ?? '',
                                  fit: BoxFit.cover,
                                  // isCircle: true,
                              ...
                              : Container(color: Colors.grey.shade200, child: const Icon(Icons.store, size: 36)),
                        ),
                      ),
                ...
                Expanded(
                  child: state.isLoading
                      ...
                      ? GestureDetector(
                          onTap: () async {
                            if (!PermissionChecker.hasPermission(AppPermissions.storeCreate)) {
                              showCustomSnackbar(
                                context: context,
                                message: l10n?.noPermissionCreateStore ?? "You don't have permission to create stores",
                                isWarning: true,
                              );
                              return;
                            }
                            ...
                            final canCreate = await limitService.canCreateStore();
                            ...
                            if (!canCreate) {
                              if (HiveStorage.activePlanId == null) {
                                ...
                                showCustomSnackbar(
                                  context: context,
                                  message: l10n?.limitReached ?? "You have reached your plan limit",
                                  isWarning: true,
                                );
                              }
                              return;
                            }
                            final result = await context.push<bool>(AppRoutes.addStore);
                          ...
                          onTap: () {
                            CustomSelectionSheet.show<Store>(
                              ...
                              items: state.stores.map((s) {
                                final bool isNotApproved = s.verificationStatus == 'not_approved';
                                final bool isDraft = s.visibilityStatus == 'draft';
                                return SelectionItem<Store>(
                                  ...
                                  onDisabledTapMessage: isNotApproved
                                      ...
                                      : (isDraft ? "your store is not visible to public yet wait for some time" : null),
                                );
                              }).toList(),
                              onSelected: (selected) {
                                context.read<StoreSwitcherCubit>().selectStore(selected);
                              },
                            );
                          },
                          child: Column(
                            ...
                            children: [
                                  ...
                                  Flexible(
                                    child: Row(
                                      children: [
                                        ...
                                        if (store?.name != null && store!.name.length > 25)
                                          const Padding(
                              ...
                              Text(
                                ...
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                                style: TextStyle(fontSize: UIUtils.caption(screenType), color: Colors.grey.shade600),
                              ),
                            ],
                          ),
                        ),
                ...
                SizedBox(width: UIUtils.gapSM(screenType)),
                if (HiveStorage.isSubscriptionAvailable &&
                    PermissionChecker.hasPermission(AppPermissions.subscriptionView))
  ...
  Widget _buildAnalysisGrid(ScreenType screenType, HomePageData? data, AppLocalizations? l10n) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final double itemWidth = (availableWidth - (horizontalGap * (crossAxisCount - 1))) / crossAxisCount;
          ...
          child: Wrap(
            ...
            children: [
              SizedBox(
                width: itemWidth,
                child: AnalysisCard(
                  label: summary?.todaysRevenue?.title ?? l10n?.earnings ?? "Earnings",
              ...
              SizedBox(
                width: itemWidth,
                child: AnalysisCard(
                  label: summary?.totalOrders?.title ?? l10n?.pendingOrders ?? "Pending Orders",
              ...
              SizedBox(
                width: itemWidth,
                child: AnalysisCard(
                  label: summary?.totalProducts?.title ?? l10n?.totalProducts ?? "Total Products",
  ...
  Widget _buildQuickActions(BuildContext context, ScreenType screenType, AppLocalizations? l10n) {
    return Padding(
      padding: UIUtils.pagePadding(screenType),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(
            ...
            style: TextStyle(fontSize: UIUtils.tileTitle(screenType), fontWeight: UIUtils.bold),
          ),
            ...
            children: [
              Expanded(
                child: _buildActionButton(
                  ...
                  onTap: () {
                    if (!PermissionChecker.hasPermission(AppPermissions.productCreate)) {
                      showCustomSnackbar(
                        context: context,
                        message: l10n?.noPermissionAddProduct ?? "You don't have permission to add product",
                        isWarning: true,
                      );
                      return;
                    }
                    ...
                    _limitService.canCreateProduct().then((canCreate) {
                      ...
                      context.push(AppRoutes.addProduct, extra: {'is_edit': false});
                      final selectedStore = context.read<StoreSwitcherCubit>().state.selectedStore;
                      if (selectedStore != null) {
                        context.read<HomePageBloc>().add(FetchHomePageData(storeId: selectedStore.id));
                      }
                    });
                  },
                ),
  ...
  Widget _buildActionButton(
    ...
    return GestureDetector(
      ...
      child: Container(
        height: 100,
        decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(UIUtils.radiusMD(screenType))),
        child: Column(
          ...
          children: [
            ...
            Text(
              label,
              style: TextStyle(color: Colors.white, fontWeight: UIUtils.bold, fontSize: UIUtils.body(screenType)),
            ),
          ],
        ),
      ),
  ...
  Widget _buildEarningsChart(BuildContext context, ScreenType screenType, HomePageData? data, AppLocalizations? l10n) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(child: CustomShimmer(width: double.infinity, height: 100)),
              SizedBox(width: 16),
              Expanded(child: CustomShimmer(width: double.infinity, height: 100)),
            ],
          ),
          ...
          Row(
            children: [
              Expanded(child: CustomShimmer(width: double.infinity, height: 180)),
              SizedBox(width: 16),
              Expanded(child: CustomShimmer(width: double.infinity, height: 180)),
            ],
          ),
  ...
  List<_ChartPoint> _getChartData(HomePageData? data) {
    ...
    if (_selectedPeriod == 'Week') {
      return data.chart!.weekly?.entries?.map((e) => _ChartPoint(e.day, e.earnings.toDouble())).toList() ?? [];
    } else if (_selectedPeriod == 'Month') {
      return data.chart!.monthly?.entries?.map((e) => _ChartPoint(e.week, e.earnings.toDouble())).toList() ?? [];
