    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        if (status == 'pending' && data['payment_url'] != null) {
          final cashfree = data['cashfree'];
          if (cashfree != null) {
            emit(
              BuySubscriptionCashfreePending(
                orderId: cashfree['order_id'].toString(),
                paymentSessionId: cashfree['payment_session_id'].toString(),
                environment: cashfree['environment']?.toString() ?? 'sandbox',
              ),
            );
            return;
          }
