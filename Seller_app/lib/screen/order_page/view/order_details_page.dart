...
class _OrderDetailsPageState extends State<OrderDetailsPage> {
  ...
  bool _isItemSelectable(OrderItemDetail item, String? selectionMode) {
    ...
    if ({'rejected', 'cancelled', 'preparing', 'ready_for_pickup', 'completed'}.contains(status)) {
      return false;
    }
    // No mode yet — any actionable item can start selection
    if (selectionMode == null) {
      return status == 'pending' || status == 'awaiting_store_response' || status == 'accepted';
    }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<OrderDetailsBloc, OrderDetailsState>(
      ...
      builder: (context, state) {
        Widget? bottomBar;
        if (state is OrderDetailsLoaded) {
          bottomBar = _buildBottomBar(context, state.orderData, state.isUpdating, l10n);
        }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildSection(
                          ...
                          child: Column(
                            children: [
                              _buildKvRow(context, l10n?.orderNumber ?? 'Order Number :', data.uuid),
                              _buildKvRow(context, l10n?.orderDate ?? 'Order Date :', data.createdAt),
                              _buildKvRow(context, l10n?.statusLabel ?? 'Status :', data.status, isStatus: true),
                              _buildKvRow(
                                context,
                              ...
                              _buildKvRow(
                                ...
                                data.paymentMethod.toUpperCase().replaceAll('_', ' '),
                              ),
                              _buildKvRow(
                                context,
                              ...
                              if (data.orderNote != null && data.orderNote!.isNotEmpty)
                                _buildKvRow(context, l10n?.orderNote ?? 'Order Note :', data.orderNote!),
                            ],
                          ),
                        ),
                        ...
                        _buildSection(
                          context,
                          title: '${l10n?.orderItems ?? "Order Items"} (${data.items?.length ?? 0})',
                          ...
                          child: Column(
                            children: () {
                              ...
                              return items.asMap().entries.map((entry) {
                                ...
                                final bool isSame = item.product?.title == item.variant?.title;
                                final itemId = item.orderItem?.id ?? item.id;
                                ...
                                return _ExpandableOrderItem(
                                  ...
                                  isSelectable: _isItemSelectable(item, selectionMode),
                                  showCheckbox: _isItemActionable(item),
                                  onSelectionChanged: (_) => _toggleItemSelection(itemId),
                                );
                              }).toList();
                            }(),
                          ),
                        ...
                        _buildSection(
                          context,
                          title: l10n?.customerInformation ?? 'Customer Information',
                          child: Column(
                            children: [
                              _buildKvRow(context, l10n?.customerName ?? 'Customer Name :', data.billingName),
                              _buildKvRow(context, l10n?.phoneLabel ?? 'Phone :', data.billingPhone),
                              _buildKvRow(context, l10n?.emailLabel ?? 'Email :', data.email),
                            ],
                          ),
                        ),
                        const SizedBox(height: 16),
  ...
  Widget _buildSection(BuildContext context, {required String title, required Widget child, Widget? trailing}) {
    final theme = Theme.of(context);
  ...
  Widget _buildKvRow(
    ...
    if (isStatus || isPaymentStatus) {
      ...
      if (v.contains('pending') || v.contains('awaiting') || v.contains('partially')) {
        color = Colors.orange;
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [CustomShimmer(width: 100, height: 16), CustomShimmer(width: 150, height: 16)],
                ),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [CustomShimmer(width: 100, height: 16), CustomShimmer(width: 150, height: 16)],
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [CustomShimmer(width: 100, height: 16), CustomShimmer(width: 80, height: 16)],
                ),
              ],
            ),
          ),
  ...
  Widget? _buildBottomBar(BuildContext context, OrderDetailsData data, bool isUpdating, AppLocalizations? l10n) {
          ...
          children: [
            Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(TablerIcons.info_circle, size: 14, color: theme.colorScheme.primary),
              ...
              children: [
                Expanded(
                  child: OutlinedButton(
                    onPressed: isUpdating ? null : () => _handleReject(context, l10n),
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: isUpdating ? null : () => _handleAccept(context, l10n),
          ...
          children: [
            Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(TablerIcons.info_circle, size: 14, color: AppColors.primaryColor),
                  const SizedBox(width: 8),
                  Text(
                    '$selectedCount item${selectedCount > 1 ? 's' : ''} selected',
                    style: theme.textTheme.labelMedium?.copyWith(
            ...
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                onPressed: isUpdating ? null : () => _handlePrepare(context, l10n),
                style: ElevatedButton.styleFrom(
...
class _ExpandableOrderItemState extends State<_ExpandableOrderItem> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Container(
          color: widget.isSelected ? theme.colorScheme.primary.withValues(alpha: 0.05) : null,
          child: InkWell(
            onTap: () {
              setState(() {
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      if (widget.showCheckbox)
                        GestureDetector(
                          onTap: widget.isSelectable ? () => widget.onSelectionChanged?.call(!widget.isSelected) : null,
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                color: widget.isSelected ? AppColors.primaryColor : Colors.grey.withValues(alpha: 0.4),
                                width: 1.5,
                              ),
                            ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              widget.item.product?.title ?? (l10n?.notAvailable ?? 'N/A'),
                            ...
                            Row(
                              children: [
                                Text(
                                  l10n?.qtyWithCount(widget.item.quantity) ?? 'Qty: ${widget.item.quantity}',
                            ...
                            if ((widget.item.addons)?.isNotEmpty ?? false) ...[
                              const SizedBox(height: 6),
                              Text(
                                "${l10n?.addOns ?? "Add-Ons"} :",
                                style: theme.textTheme.bodySmall?.copyWith(
                              ...
                              ...List.generate((widget.item.addons?.length) ?? 0, (addonsIndex) {
                                final addon = widget.item.addons![addonsIndex];
                                final groupTitle = addon.group?.title ?? (l10n?.addOns ?? "Add-On");
                                final itemTitle = addon.item?.title ?? "";

                                return Text(
                                  '$groupTitle: $itemTitle - ${widget.currencySymbol}${addon.price}',
                                  style: theme.textTheme.bodySmall?.copyWith(
                                    fontSize: 10,
                                    fontWeight: FontWeight.w500,
                                    color: theme.colorScheme.onSurface.withValues(alpha: 0.6),
                            ...
                            if ((widget.item.orderItem?.status ?? '').isNotEmpty) ...[
                              const SizedBox(height: 8),
                              _buildStatusChip(context, widget.item.orderItem!.status),
                            ],
                          ],
                        ),
                      ),
                        ...
                        children: [
                          if (!widget.isSame)
                            _buildDetailRow(
                              ...
                              widget.item.variant?.title ?? (l10n?.notAvailable ?? 'N/A'),
                            ),
                          _buildDetailRow(context, l10n?.quantity ?? 'Quantity', widget.item.quantity.toString()),
                          _buildDetailRow(
                            context,
                            l10n?.price ?? 'Price',
                          ...
                          if ((widget.item.addons)?.isNotEmpty ?? false)
                            _buildDetailRow(
                              context,
                              l10n?.addOns ?? "Add-Ons",
                              '${widget.currencySymbol} ${widget.item.addonsTotal.toStringAsFixed(2)}',
                            ),

                          _buildDetailRow(
                            context,
                            l10n?.subtotal ?? 'Subtotal',
                            '${widget.currencySymbol} ${widget.item.subTotal}',
                            isLast: true,
                          ),
                            ...
                            SizedBox(
                              ...
                              child: ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  final attachment = widget.item.attachments![index];
                                      ...
                                      child: isImage
                                          ? CachedNetworkImage(
                                              ...
                                              placeholder: (context, url) =>
                                                  const Center(child: CustomShimmer(width: 80, height: 80)),
  ...
  Widget _buildStatusChip(BuildContext context, String status) {
    ...
    return Container(
      ...
      child: Text(
        v.toUpperCase(),
        style: theme.textTheme.labelSmall?.copyWith(color: color, fontWeight: FontWeight.w600, fontSize: 9),
      ),
    );
  }
}
