...
import '../screen/more_page/view/store_addon_inventory/store_addon_list/cubit/store_addon_inventory_cubit.dart';
import '../screen/more_page/view/subscription_plans/bloc/buy_subscription/buy_subscription_event.dart';
import '../screen/products_page/add_ons/addon_groups/cubit/addon_groups_cubit.dart';
import '../screen/products_page/product_addons/product_addons_group/cubit/product_addon_cubit.dart';
...
class MasterApiService {
  ...
  static Future<void> callNeededApisOnLogin(BuildContext context) async {
    ...
    // 1. Profile (Global)
    context.read<ProfileBloc>().add(const ProfileFetchEvent(forceRefresh: true));

    // 2. Current Subscription & History
    context.read<CurrentSubscriptionBloc>().add(FetchCurrentSubscription());
    context.read<SubscriptionHistoryBloc>().add(LoadSubscriptionHistoryInitial());
    ...
    //10. AddOns
    context.read<AddonGroupsCubit>().fetchAddonGroups();

    context.read<ProductAddonCubit>().fetchProductAddon();
    context.read<StoreAddonInventoryCubit>().fetchInventory();
  }
  ...
  static void clearAllApisData(BuildContext context) {
    ...
    // ── Addons ──
    context.read<StoreAddonInventoryCubit>().reset();
  }
}
