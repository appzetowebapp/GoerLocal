...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _verifyPhoneNumber({
    ...
    if (AppHelpers.smsGatewayIsFirebase) {
      ...
      log('Sending Custom OTP to: $phoneNumber');
      try {
        await _repository.sendCustomOTP(mobile: phoneNumber);
        log('✅ Custom OTP Sent!');
        add(OnPhoneOtpSend(
            verificationId: 'CUSTOM_SMS', resendToken: 0, isLogin: isLogin));
      } catch (e) {
        log('Custom OTP Send Failed: $e');
        add(AuthFailureEvent(error: 'Failed to send OTP. Please try again.'));
      }
    }
  }
  ...
  Future<void> _onSocialAuthRequest(
    ...
    try {
      ...
      if (response['success'] == false &&
        ...
        emit(AuthFailed(error: response['message']?.toString() ?? 'Login blocked.'));
      }
    } catch (e) {
      emit(AuthFailed(error: e.toString()));
    }
