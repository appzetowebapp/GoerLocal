    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (final item in rawSuggestions) {
          ...
          final address =
              secondaryText.isNotEmpty ? '$mainText, $secondaryText' : fullText;
    ...
    try {
        ...
        options: Options(
          headers: {
            'X-Goog-Api-Key': Platform.isAndroid
                ...
                : AppConstant.iosMapKey,
            'X-Goog-FieldMask':
                'location,formattedAddress', // only request what you need
            'Content-Type': 'application/json',
            // Optional: 'Referer': 'com.goerlocal.ordering',  // usually not required for server-side calls
          },
        ),
      );
      ...
      if (response.statusCode == 200) {
        ...
        final formattedAddress =
            data['formattedAddress'] as String? ?? fallbackAddress;
        ...
        _showErrorSnackBar(
            'Failed to fetch location details (HTTP ${response.statusCode})');
      }
    } catch (e) {
      setState(() {
        _isSearching = false;
