...
class _DeliveryTrackingPageState extends State<DeliveryTrackingPage>
  ...
  LatLng?
      _pickupLocation; // first uncollected store (or customer if all collected)
  ...
  List<RouteDetail> _getPendingStoreStops(List<RouteDetail> allStops) {
    return allStops.where((s) {
      ...
      final isCustomer = name.contains('customer') || name.contains('custom');
  ...
  Widget _buildMainUI(BuildContext context, DeliveryTrackingState state) {
    ...
    if (order != null && order.items.isNotEmpty) {
      otpText = order.items.first.otp?.toString();
    }
    ...
    return FlutterMap(
      ...
      children: [
        TileLayer(
          ...
          userAgentPackageName: 'com.goerlocal.ordering',
        ),
        PolylineLayer(
          polylines: [
