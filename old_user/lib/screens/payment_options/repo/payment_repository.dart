  ...
  Future<Map<String, dynamic>> initiatePayment({
    ...
    try {
      switch (paymentMethodType) {
        ...
        case PaymentMethodType.cashfreePayment:
          return await _initiateCashfreePayment(
              context: context,
              amount: amount,
              additionalData: additionalData,
              addMoneyToWallet: addMoneyToWallet);
  ...
  Future<Map<String, dynamic>> _initiateRazorpayPayment({
    ...
    try {
      ...
      if (addMoneyToWallet) {
        ...
        response =
            await AppHelpers.apiBaseHelper.postAPICall(ApiRoutes.razorpayApi, {
          "amount": amount,
          "currency": 'INR',
...
// ============================================================================
  ...
  Future<Map<String, dynamic>> _initiateFlutterWavePayment({
    ...
    try {
      ...
      if (addMoneyToWallet) {
        response = await AppHelpers.apiBaseHelper
  ...
  Future<Map<String, dynamic>> _initiateCashfreePayment({
    required BuildContext context,
    required double amount,
    Map<String, dynamic>? additionalData,
    required bool addMoneyToWallet,
    ...
    try {
      ...
      if (addMoneyToWallet) {
        response = await AppHelpers.apiBaseHelper
            .postAPICall(ApiRoutes.prepareWalletRechargeApi, {
          "amount": amount.toInt(),
          "payment_method": 'cashfreePayment',
          "description": 'Wallet recharge'
        });
      } else {
        response = await AppHelpers.apiBaseHelper
            .postAPICall(ApiRoutes.cashfreeCreateOrderApi, {
          'amount': amount,
        });
      }
      ...
      if (response.data['success'] != true) {
        return {
          'success': false,
          'error':
              response.data['message'] ?? 'Failed to create Cashfree order',
        };
      }

      final Map<String, dynamic> cfData = addMoneyToWallet
          ? response.data['data']['payment_response']
          : response.data['data'];

      final String orderId = cfData['order_id'] ?? '';
      final String paymentSessionId = cfData['payment_session_id'] ?? '';

      if (orderId.isEmpty || paymentSessionId.isEmpty) {
        return {
          'success': false,
          'error': 'Invalid Cashfree order or session ID'
        };
      }

      final paymentSettings = SettingsData.instance.payment;
      final envString = paymentSettings?.cashfreePaymentMode == 'live'
          ? 'production'
          : 'sandbox';
      final CFEnvironment env = envString == 'production'
        ...
        (String paidOrderId) {
          if (!paymentCompleter.isCompleted) {
            paymentCompleter.complete({
              ...
              'payment_id': paidOrderId,
              'signature': paymentSessionId,
            });
          }
        },
        (CFErrorResponse error, String paidOrderId) {
          if (!paymentCompleter.isCompleted) {
            paymentCompleter.complete({
              'success': false,
              'error': error.getMessage() ?? 'Cashfree payment failed',
            });
          }
        },
      );
      ...
      return {
        'success': false,
        'error': 'Cashfree payment failed: ${e.message}'
      };
    } catch (e) {
      return {
        'success': false,
        'error': 'Cashfree payment failed: ${e.toString()}'
      };
    }
  }
}
