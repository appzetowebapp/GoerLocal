  ...
  Future<void> _onFetchListingProducts(FetchListingProducts event, Emitter<ProductListingState> emit) async {
    ...
    try {
      ...
      if(response['data'].isNotEmpty) {
        ...
        totalProducts = int.tryParse(response['data']['total']?.toString() ?? '0') ?? 0;
        final currentTotal = int.tryParse(response['data']['current_page']?.toString() ?? '1') ?? 1;
        final lastPageNum = int.tryParse(response['data']['last_page']?.toString() ?? '1') ?? 1;
